...
class ExampleAutonomyProtocol : public AutonomyProtocol {
 public:
  ...
  // and uses these for initialization. In particular, it initializes its parent
  // class, AutonomyProtocol, as well as the occupancy grid and arena graph.
  ExampleAutonomyProtocol(
    ...
    // Important hint: graph_of_arena will be large (many nodes) if cell_size_
    // (defined below) is small.  If you naively extend your AStar2D function to
    // 3D, it will run slowly on a large graph_of_arena.  Of course, you can
    // keep the graph small by choosing a large cell_size, but this may prevent
    // you from finding paths through areas with tight obstacle spacing.  A
    // better approach is to speed up A*.  Note that its most costly operation
    // is the search through the container of explored nodes to determine
    // whether a candidate node has already been explored.  You can speed up
    // this search dramatically by making this container an std::unordered_set
    // rather than an std::vector.  See
    // https://how.dev/answers/unordered-sets-in-cpp
