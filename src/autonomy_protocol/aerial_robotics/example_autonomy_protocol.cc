...
// BuildHistoryVectors is a member function of the ExampleAutonomyProtocol class
// and is used to produce vectors containing samples of the trajectory generated by P4.
void ExampleAutonomyProtocol::BuildHistoryVectors(  std::vector<double>& x_hist,
...
ExampleAutonomyProtocol::UpdateTrajectories() {
  ...
  //
  //        const std::chrono::time_point<std::chrono::system_clock> timepointVar
  ...
  // like Unix. For more information, visit https://cplusplus.com/reference/chrono/.

  // First live examples of using std::chrono are below.
  // You can declare any unit of time.
  static std::chrono::milliseconds dt_chrono_u_ =
  ...
  bool change_start_pos = false; // To check whether the drone has changed the start position or not
  ...
  if (first_time_) {
    ...
    change_start_pos = true; // To move start position on purpose only for first time
  }
  ...
  const std::chrono::time_point<std::chrono::system_clock> current_chrono_time_t_ =
      std::chrono::system_clock::now();
  ...
  while (bad_pos == true) {
    ...
    // lower_bound, upper_bound) In this example, node inequality bounds are used
    // to constrain the trajectory positions in order to hit the balloons. These
    // bounds specify 0.2m x 0.2m x 0.2m boxes centered around the balloon
    // centers.
    const double p_bound = 0.1;
    std::vector<p4::NodeInequalityBound> node_inequality_bounds;
    for (int w = 1; w < waypoints.size(); w++) {
      node_inequality_bounds.push_back(p4::NodeInequalityBound(0,w,0,waypoints[w].x()-p_bound,waypoints[w].x()+p_bound));
      node_inequality_bounds.push_back(p4::NodeInequalityBound(1,w,0,waypoints[w].y()-p_bound,waypoints[w].y()+p_bound));
      node_inequality_bounds.push_back(p4::NodeInequalityBound(2,w,0,waypoints[w].z()-p_bound,waypoints[w].z()+p_bound));
    }

    // A segment is defined to be the path connecting two consecutive nodes. A
    // segment inequality bound the following linear inequality as a constraing on
    // the segment: dot(a,x) < b. The x vector is the derivative vector, the a
    // vector is its corresponding coefficient vector, and the scalar b is a
    ...
    for (int w = 0; w < waypoints.size()-1; w++){
      segment_inequality_bounds.push_back(p4::SegmentInequalityBound(w,1,Eigen::Vector3d(1,0,0),v_bound));
      segment_inequality_bounds.push_back(p4::SegmentInequalityBound(w,1,Eigen::Vector3d(0,1,0),v_bound));
      segment_inequality_bounds.push_back(p4::SegmentInequalityBound(w,1,Eigen::Vector3d(0,0,1),v_bound));
      segment_inequality_bounds.push_back(p4::SegmentInequalityBound(w,1,Eigen::Vector3d(-1,0,0),v_bound));
      segment_inequality_bounds.push_back(p4::SegmentInequalityBound(w,1,Eigen::Vector3d(0,-1,0),v_bound));
      segment_inequality_bounds.push_back(p4::SegmentInequalityBound(w,1,Eigen::Vector3d(0,0,-1),v_bound));
    }
    ...
    { 
      ...
      BuildHistoryVectors(x_hist, y_hist, z_hist, times, path, sampler_options);

      sampler_options.derivative_order = 1;
      std::vector<double> vx_hist, vy_hist, vz_hist;
      BuildHistoryVectors(vx_hist, vy_hist, vz_hist, times, path, sampler_options);

      sampler_options.derivative_order = 2;
      std::vector<double> ax_hist, ay_hist, az_hist;
      BuildHistoryVectors(ax_hist, ay_hist, az_hist, times, path, sampler_options);
      ...
      for(size_t time_idx = 0; time_idx < x_hist.size(); ++time_idx) {
        ...
        trajectory_vector.push_back((Eigen::Matrix<double, 11, 1>() << x_hist[time_idx], y_hist[time_idx], z_hist[time_idx],
                                                                       vx_hist[time_idx], vy_hist[time_idx], vz_hist[time_idx],
                                                                       ax_hist[time_idx], ay_hist[time_idx], az_hist[time_idx],
                                                                       yaw, flight_time)
                                                                       .finished());
      }
    ...
    // trajectory before mediation layer ever has to see it. Assuming it doesn't
    // pass the final check, you can jump back up to a certain part of the code
    // through a variety of means, where you can then compensate for the error
    // and try again. An example has been included that shows how one might
    // handle a bad starting position with a while loop on the trajectory
    // generation step. Remove fields and lines of code related to bad_pos and

    // Further details that might help you understand how to utilize all features
    // of the prevetting: This interface's source code can be found in
    // presubmission_trajectory_vetter.h. The PreVet function returns type
    // TrajectoryCode which contains three values: (1) The mediation layer code
    // that specifies success or the failure; (2) The failure value (e.g., if the
    // velocity limit is 4.0 m/s and you submit 5.0 m/s, the value is returned as
    // 5.0); (3) The failure index. This is the trajectory sample index that
    // caused the mediation layer to kick back an error code. The index is the
    // sampled index (specified from the P4 sampling process). You can figure out
    // the waypoint index by a simple math transformation.
    ...
    switch (prevetter_response.code) {
      ...
      case MediationLayerCode::StartPointFarFromCurrentPosition: {
            std::cout << "Prevet: Start point far from current position" << std::endl;
            std::cout << "Prevet: Position violation: " << prevetter_response.value
                      << std::endl;
            bad_pos = true;
            break;
      }
    ...
    // To break out the while loop, if prevet(StartPointFarFromCurrentPosition) is passed
    if (bad_pos == false) {
        break;
    }
    // Sleep for 5 seconds and make warning during start position is located on weird position.
    else {
