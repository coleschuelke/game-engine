...
namespace game_engine
{
  ...
  namespace
  {
    ...
    // Wrap a node
    std::shared_ptr<NodeWrapper> wrap_node(const std::shared_ptr<NodeWrapper> parent, const std::shared_ptr<Node2D> current_node, const std::shared_ptr<Node2D> end_node, const double cost)
    {
      auto wrapped_node = std::make_shared<NodeWrapper>();
      wrapped_node->parent = parent;
      wrapped_node->node_ptr = current_node;
      wrapped_node->cost = cost;
      wrapped_node->heuristic = heuristic(current_node, end_node);
                        ...
                        const std::shared_ptr<Node2D> end_ptr)
  {
    ...
    NodeWrapperPtr start_nwptr = wrap_node(nullptr, start_ptr, end_ptr, 0);
    ...
    while (!nodes_to_explore.empty())
    {
      ...
      for (const auto edge : graph.Edges(node_to_explore->node_ptr))
      {
        ...
        NodeWrapperPtr new_nwptr = wrap_node(node_to_explore, sink_ptr, end_ptr, node_to_explore->cost + cost);
